        ...
        {knowledgeItems.length > 0 ? (
          knowledgeItems.map((item: Knowledge, index: number) => (
            <button
              ...
              className="w-full text-left p-3 rounded-lg bg-amber-50/50 hover:bg-amber-50 border border-amber-100 hover:border-amber-200 transition-all duration-150 group cursor-pointer relative animate-fade-in-up"
              style={{ animationDelay: `${index * 50}ms` }}
            >
              <p className="text-sm text-gray-700 line-clamp-2 mb-2 group-hover:text-gray-900 transition-colors">
                {item.content}
              </p>
              ...
              <button
                ...
                className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 p-1.5 rounded-full bg-white/80 hover:bg-red-50 text-gray-400 hover:text-red-500 transition-all shadow-sm"
